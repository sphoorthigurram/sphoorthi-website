        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {certifications.map((cert, i) => (
            <Reveal key={cert.id} delay={i * 90} as="article">
              <div
                className={cn(
                  "group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface transition-all",
                ...
                {cert.image ? (
                  <div className="relative aspect-[4/3] w-full border-b border-border bg-background">
                    <MediaLightbox
                      ...
                      className="h-full rounded-none border-none [&_img]:h-full [&_img]:object-contain"
                    />
                  </div>
                ) : (
