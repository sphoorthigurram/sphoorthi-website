        ...
        {projects.map((project, i) => (
          <Reveal key={project.id} delay={i * 90}>
            <button
              ...
              className="group relative flex h-full w-full flex-col overflow-hidden rounded-2xl text-left panel transition-all duration-500 hover:-translate-y-1 hover:glow-ring"
            >
          ...
          {active ? (
            <div>
              ...
              <div className="p-6 sm:p-8">
                ...
                <DialogTitle className="mt-2 text-2xl font-semibold sm:text-3xl">
                  {active.title}
                </DialogTitle>
                  ...
                  <section className="mt-8 border-t border-border pt-7">
                    ...
                    <div className="mt-4 grid gap-4">
                      {active.diagrams.map((diagram) => (
                        <article key={diagram.image} className="rounded-lg border border-border bg-surface p-3">
                          <MediaLightbox
                            src={diagram.image}
                            alt={diagram.title}
                            caption={diagram.description}
                            className="max-h-[26rem] bg-background [&_img]:max-h-[26rem] [&_img]:object-contain"
                          />
                          <div className="px-1 pb-1 pt-4">
                            <div className="flex flex-wrap items-center gap-2">
                              <h5 className="text-base font-semibold">{diagram.title}</h5>
                              <span className="rounded-md bg-primary/10 px-2 py-1 font-mono text-[9px] uppercase tracking-widest text-primary">
                                Click to expand
                              </span>
                            </div>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      {active.externalLinks.map((link) => (
                        <Button
                          ...
                          asChild
                          variant="outline"
                          className="h-auto min-h-20 justify-between whitespace-normal p-4 text-left"
                        >
...
export function Gallery() {
  ...
  return (
    <section id="gallery" className="scroll-mt-24 border-y border-border bg-surface py-24">
          ...
          {gallery.map((photo, i) => (
            <Reveal key={photo.src} delay={(i % 3) * 80} className="mb-4 break-inside-avoid">
              ...
              >
                ...
                />
                <span className="absolute inset-x-0 bottom-0 translate-y-2 bg-gradient-to-t from-background/95 to-transparent p-4 text-left text-xs leading-snug text-foreground opacity-0 transition-all duration-400 group-hover:translate-y-0 group-hover:opacity-100">
                  {photo.caption}
                </span>
              </button>
            </Reveal>
      ...
      {index !== null ? (
        <div
          className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-background/95 p-4 backdrop-blur-md animate-in fade-in"
          role="dialog"
          aria-modal="true"
        >
          <button
            ...
            className="absolute right-5 top-5 grid size-10 place-items-center rounded-lg border border-border bg-surface text-muted-foreground hover:text-primary"
          >
            <X className="size-4" />
          </button>
          <button
            ...
            className="absolute left-4 grid size-11 place-items-center rounded-full border border-border bg-surface/80 text-muted-foreground hover:text-primary"
          >
          ...
          <button
            ...
            className="absolute right-4 grid size-11 place-items-center rounded-full border border-border bg-surface/80 text-muted-foreground hover:text-primary"
          >
